...
export const description: INodeProperties[] = [
  {
    ...
    displayOptions: { show: { resource: ['pipeline'], operation: ['deletePipeline'] } },
  },
];
...
export async function execute(
  ...
  for (let i = 0; i < items.length; i++) {
    try {
      ...
      await apiRequest.call(this, 'DELETE', `/pipelines/${pipelineId}`);

      returnData.push(
        ...this.helpers.constructExecutionMetaData(
          this.helpers.returnJsonArray({ success: true, pipelineId }),
          { itemData: { item: i } },
        ),
      );
    } catch (error) {
