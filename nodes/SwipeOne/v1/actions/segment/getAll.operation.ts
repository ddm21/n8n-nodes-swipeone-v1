...
export const description: INodeProperties[] = [
  {
    ...
    displayOptions: { show: { resource: ['segment'], operation: ['getAll'] } },
  },
  {
    displayName: 'Limit',
    name: 'limit',
    ...
    displayOptions: { show: { resource: ['segment'], operation: ['getAll'], returnAll: [false] } },
  },
];
...
export async function execute(
  ...
  for (let i = 0; i < items.length; i++) {
    try {
      ...
      let segments: IDataObject[] = response?.data?.segments ?? response?.data ?? [];
      ...
      returnData.push(
        ...this.helpers.constructExecutionMetaData(this.helpers.returnJsonArray(segments), {
          itemData: { item: i },
        }),
      );
    } catch (error) {
